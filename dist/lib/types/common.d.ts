...
export type Keys<TDocument extends Document> = {
    [K in keyof TDocument as K & string]: TDocument[K];
};
export type Keywords<TDocument extends Document> = {
    [K in keyof TDocument as `${K & string}.keyword`]: TDocument[K];
};
export type Fields<TDocument extends Document> = {
    [K in keyof TDocument as K & string | `${K & string}.keyword`]: TDocument[K];
};
